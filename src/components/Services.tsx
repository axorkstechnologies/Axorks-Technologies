    ...
    <section className="w-full px-4 sm:px-6 lg:px-8 py-20 lg:py-28 relative" id="services">
      <div className="max-w-7xl mx-auto relative z-10">
            ...
            return (
              ...
              >
                    ...
                    {service.features.map((feature, fi) => (
                      <div key={fi} className="flex items-start gap-2.5 text-xs text-[var(--text-muted)]">
                        <div
                          ...
                          style={{ backgroundColor: accent.bgLight }}
                        >
                          <Check className="w-2.5 h-2.5" style={{ color: accent.color }} />
                        </div>
                        <span className="leading-snug text-[var(--text-secondary)]">{feature}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-8 pt-4 border-t border-[var(--glass-border)] flex items-center justify-between">
                  <span className="text-[11px] font-mono-code text-[var(--gold)] font-semibold">
                    Starting from $1,000
                  </span>

                  <a
                    href="#contact"
                    className="inline-flex items-center gap-1.5 text-xs font-headline font-bold uppercase tracking-wider group-hover:translate-x-1 transition-all cursor-pointer"
        ...
        <div className="mt-14 sm:mt-16 text-center">
          <div className="inline-flex flex-wrap items-center justify-center gap-3 px-6 py-3 rounded-full glass-2 border-[var(--glass-border)] shadow-lg text-sm text-[var(--text-secondary)]">
            <span>Every project receives an exact fixed-price scope.</span>
            <span className="text-[var(--gold)] font-bold font-mono-code">Milestones start from $1,000.</span>
            <span className="text-[var(--text-muted)] hidden sm:inline">•</span>
            <span className="text-[var(--emerald)] font-medium">100% Commercial IP Transfer</span>
          </div>
        </div>
      </div>
    </section>
